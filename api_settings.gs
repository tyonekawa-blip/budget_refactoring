...
function getRawDataPackage(targetPeriod, targetTeamCd) {
  try {
    ...
    // === 【四半期・期間指定の動的展開ロジック修正済】 ===
    let targetFY; 
    let reportTargetMonths = [];
    
    if (targetPeriod.includes('_')) {
      const parts = targetPeriod.split('_'); 
      targetFY = parseInt(parts[0], 10);
      const type = parts[1]; 
      
      const qStr = config['QUARTER_' + type];
      if (qStr) {
        const monthsStrArray = qStr.split(',').map(m => m.trim()).filter(m => m);
        reportTargetMonths = monthsStrArray.map(mStr => {
           let m = parseInt(mStr, 10);
           // 10月期首ルール：10〜12月は当年(targetFY - 1)、1〜9月は當年(targetFY)
           let y = m >= 10 ? targetFY - 1 : targetFY;
           return `${y}${m.toString().padStart(2, '0')}`;
        });
      } else {
        const getMonths = (startM, count) => { 
          const res = []; 
          for(let i=0; i<count; i++) { 
            let m = startM + i; 
            let y = targetFY;
            if(m > 12) { y++; m -= 12; }
            if(m >= 10) y = targetFY - 1; 
            res.push(`${y}${m.toString().padStart(2, '0')}`); 
          } 
          return res;
        };
        if (type === '1Q') reportTargetMonths = getMonths(10, 3);
        else if (type === '2Q') reportTargetMonths = getMonths(1, 3);
        else if (type === '3Q') reportTargetMonths = getMonths(4, 3);
        else if (type === '4Q') reportTargetMonths = getMonths(7, 3);
        else if (type === 'H1') reportTargetMonths = getMonths(10, 6);
        else if (type === 'H2') reportTargetMonths = getMonths(4, 6);
        else if (type === 'ALL') reportTargetMonths = getMonths(10, 12);
      }
    } else { 
      targetFY = getFiscalYear(targetPeriod); 
      reportTargetMonths = [targetPeriod];
    }
    
    const targetMonths = []; const prevTargetMonths = [];
    for (let i = 0; i < 12; i++) { 
      let y = targetFY;
      let m = 10 + i; if (m > 12) { y += 1; m -= 12; } 
    ...
    function extractData(sheetName, isCurrentYear) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (rowType === '予算' || rowType === labelBudget || rowType.includes('予算')) { 
          ...
          if (actSet.includes('速報') && rowType.includes('速報')) {
            useRow = true;
            outputType = '速報実績';
          } else if (actSet.includes('確定') && rowType.includes('確定')) {
            useRow = true;
            outputType = '確定実績';
          } else if (rowType === '実績') {
            useRow = true;
